...
// Command is the concrete command type KVService submits to the Raft log to manage
// its state machine. It's also used to carry the results of the command after
// it's applied to the state machine. These are the supported commands:
//
// CommandGet: queries a key's value
//
// * Key is the key to get, Value is ignored
// * CompareValue is ignored
// * ResultFound is true if Key was found in the store
