...
import (
	...
	"math/rand/v2"
	"net/http"
	kvclient "raft/key-value/kv-client"
	kvservice "raft/key-value/kv-service"
	"raft/raft"
	"strings"
	"testing"
	"time"
...
func NewHarness(t *testing.T, n int) *Harness {
	...
	for i := range n {
		...
		kvss[i] = kvservice.NewService(i, peerIds, storage[i], ready)
		alive[i] = true
	}
...
func (h *Harness) RestartService(id int) {
	...
	ready := make(chan any)
	h.kvCluster[id] = kvservice.NewService(id, peerIds, h.storage[id], ready)
...
func (h *Harness) NewClient() *kvclient.KVClient {
	...
	return kvclient.NewClient(addrs)
}
